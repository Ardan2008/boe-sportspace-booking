...
- **Passwords stored in plaintext** (`admins.password`). This is intentional for the project.
- Admin login URL is dynamic: `/{ADMIN_LOGIN_SECRET}` — set via `ADMIN_LOGIN_SECRET` env var (read from `config/services.php`).
- Middleware `admin.access` (aliased in `bootstrap/app.php`) checks session. Supports `owner` and `can_edit` requirements:
...
| `app/Models/` | 11 models: Booking, Fasilitas, Admins, Penyewa, JadwalBlokir, AuditLog, etc. |
...
- Turnstile/recaptcha only verified in `production` environment
- PDF receipts via `barryvdh/laravel-dompdf`
- Sends email on approve/reject (`BookingApprovedMail` / `BookingRejectedMail`) — mails are queued
- `php artisan storage:link` required for public access to uploaded images

## Important conventions

- **No comments in code** — do not add PHP/BLADE/JS comments unless necessary.
